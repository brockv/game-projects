...
if (outside_room(o_player, 200)) { // Run script to check the player's position
	player_death = 1; // The player is dead
	screenshake	 = 1; // Activate screenshake	
}
...
if (k_charge_pressed) {
	if (effect_shine_timer_ > minimum_radius_) {
		// Only drain the light gauge if the player is moving
		if (hspd_ != 0 || (!player_on_ground_ && !player_on_jt_)) {
