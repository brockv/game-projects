/// @function						Aura_Light_Set_Radius(light_instance, radius_, [scale]);
/// @description					Set the radius_ of a light caster instance
/// @param {real}	light_instance	The instance to set the radius_ of
/// @param {real}	radius_			The new radius_ value
/// @param {bool}	scale			OPTIONAL! Whetner to scale the sprite too or not

// This setter script permits you to set the radius_ variable for a light caster.
// You supply the light instance to set, and then the radius_ value, which must be
...
// Note, when you change the light radius_ you are destroying and recreating the 
// light surface and as such the variable "aura_light_update" will be set to 
// true as well so the surface gets the updated shadow mesh for the new radius_.
